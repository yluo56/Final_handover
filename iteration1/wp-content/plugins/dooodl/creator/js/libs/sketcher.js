function Sketcher( canvasID, brushImage, $) {
	...
	this.touchSupported = 'ontouchstart' in window || navigator.msMaxTouchPoints;
...
Sketcher.prototype.setEnabled = function(enabled){
	if(enabled==true){
		this.enabled=true;
	}
	else{
...
Sketcher.prototype.getEnabled = function(){
	return this.enabled;
}

Sketcher.prototype.setBrush = function(newBrush){
	var self = this;
	this.brush = newBrush;
	this.renderFunction = (newBrush == null || newBrush == undefined) ? this.updateCanvasByLine : this.updateCanvasByBrush;
...
Sketcher.prototype.onCanvasMouseDown = function () {
	...
	return function(event) {
		...
		jQuery(document).bind( self.mouseMoveEvent, self.mouseMoveHandler );
		jQuery(document).bind( self.mouseUpEvent, self.mouseUpHandler );
...
Sketcher.prototype.onCanvasMouseUp = function (event) {
	...
	return function(event) {
		...
		jQuery(document).unbind( self.mouseMoveEvent, self.mouseMoveHandler );
		jQuery(document).unbind( self.mouseUpEvent, self.mouseUpHandler );
