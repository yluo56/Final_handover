    ...
    final class Joy_Of_Text_Plugin_Settings {
        ...
        */
        public function render_groupmembers($sections, $tab, $grpid) {
            ...
            foreach ( $groupmembers as $groupmember ) 
            {
                ...
                $html .= "<div class=\"divider\"></div>";
                $html .= "<a href='#' id='jot-mem-delete-" . $groupmember->jot_grpid . '-' . $groupmember->jot_grpmemid . "'><img src='" . plugins_url( 'images/delete.png', dirname(__FILE__) ) .  "' title='Delete'></a><div class=\"divider\"></div></td>";     
                $html .= "<div class=\"divider\"></div>";
                $html .= "</td>";
        ...
        */
        protected function render_field_textarea ( $key, $args ) {
                ...
                $html = '<textarea id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" cols="' . $cols . '" rows="' . $rows. '" placeholder="' . $placeholder .'"' . $maxlength. $readonly . '>' . esc_attr( $args['value']) . '</textarea>' . "\n";
