...
class RowsListControl extends BaseControl
{
    ...
    public function filterDefault($data)
    {
        if (is_array($data)) {
            ...
            $data = str_replace('[tag_theme_uri]', apply_filters('mesmerize_assets_url', get_template_directory_uri(), '/'), $data);
    ...
    protected function render()
    {
        ...
        <script type="text/javascript">
            jQuery(function ($) {
                ...
                jQuery("#accordion-section-<?php echo esc_attr($this->id);?> .cp-add-section").click(function (event) {
                    top.ExtendThemesCompanionPopover.showPopover(event.currentTarget)
                });
            })
        </script>
        <style>
