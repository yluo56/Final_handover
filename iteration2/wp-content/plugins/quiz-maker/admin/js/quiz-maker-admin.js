(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).on("keydown", function(e){
            ...
            if(e.keyCode === 27){
                $(document).find('.modal .ays-modal-content').css('animation-name', 'zoomOut');
                setTimeout(function(){
        ...
        $(document).find('.ays-close').on('click', function () {
            $(document).find('.ays-modal-content').css('animation-name', 'zoomOut');
            setTimeout(function(){
                $(document).find('.ays-modal').modal('hide');
            }, 250);
        });
                ...
                let answerRow = $('<tr class="ays-answer-row ui-state-default">'+
                                ...
                                '<td title="This property aviable only in pro version">'+
                                   ' <label class="ays-label" for="ays-answer">'+
                                        '<a style="opacity: 0.4" href="https://ays-pro.com/index.php/wordpress/quiz-maker/" target="_blank" class="add-answer-image">Add</a>'+
                                    '</label>'+
                                '</td>'+
                                '<td>'+
        ...
        $(document).find('.ays-add-question').on('click', function () {
            $(document).find('.ays-modal-content').css('animation-name', 'zoomIn');
            $(document).find('#ays-questions-modal').modal({
                show: true,
                keyboard: false,
                backdrop: 'static'
            });
        ...
        $(document).find('.ays-delete-question').live('click', function () {
            ...
            var questions_count = Number($('#questions_count_number').text()) - 1;
            switch (questions_count) {
                case -1:
                    $('.ays_questions_count').html('<span id="questions_count_number">0</span> item');
                    break;
                case 1:
                    $('.ays_questions_count').html('<span id="questions_count_number">1</span> item');
                    break;
                default:
                    $('#questions_count_number').text(questions_count);
            }
        });
