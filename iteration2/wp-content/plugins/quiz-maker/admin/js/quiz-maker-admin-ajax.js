(function( $ ) {
        ...
        $.ajax({
            ...
            success: function(response){
                if( response.status === true ) {
                    ...
                    let table_rows = $('table#ays-questions-table tbody tr');
                    existing_ids = existing_ids.concat(new_ids);
                ...
                $(document).find('#ays-questions-modal .ays-modal-content').css('animation-name', 'zoomOut');
                setTimeout(function(){
                    $(document).find('#ays-questions-modal').modal('hide');
                }, 250);
                let questions_count = response.ids.length;

                let table_rows = $('table#ays-questions-table tbody tr');
                if(response.ids.length > 1){
                    $(document).find('.ays_questions_count').html('<span id="questions_count_number">'+table_rows.length+'</span> items');
                }else{
                    $(document).find('#questions_count_number').text(response.ids.length);
                }
    ...
    $(document).find('.ays-show-results').on('click', function(e){
        $(document).find('#ays-results-modal .ays-modal-content').css('animation-name', 'zoomIn');
        $(document).find('#ays-results-modal').modal('show');
        $(document).find('div.ays-quiz-preloader').css('display', 'flex');
        e.preventDefault();
    });
    
    $(document).find('#ays-close-results').on('click', function() {
        $(document).find('#ays-results-modal .ays-modal-content').css('animation-name', 'zoomOut');
        setTimeout(function(){
            $(document).find('#ays-results-modal').modal('hide');
        }, 250);
    });
