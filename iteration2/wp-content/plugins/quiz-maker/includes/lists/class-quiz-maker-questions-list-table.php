            ...
            <select name="filterby" id="bulk-action-selector-top">
                ...
                <?php
                    foreach($cat_titles as $key => $cat_title){
                        ...
                        echo "<option ".$selected." value='".$cat_title->id."'>".$cat_title->title."</option>";
                    }
                ?>
            </select>
...
//            var_dump($author);die();
    ...
    function column_question( $item ) {
        ...
        $actions = array(
            ...
            'delete' => sprintf( '<a href="?page=%s&action=%s&question=%s&_wpnonce=%s">'. __('Delete', $this->plugin_name) .'</a>', esc_attr( $_REQUEST['page'] ), 'delete', absint( $item['id'] ), $delete_nonce )
        );
