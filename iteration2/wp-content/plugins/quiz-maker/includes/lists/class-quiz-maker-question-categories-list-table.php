...
class Question_Categories_List_Table extends WP_List_Table{
    ...
    function column_title( $item ) {
        ...
        if(intval($item['id']) !== 1){
            $actions['delete'] = sprintf( '<a href="?page=%s&action=%s&question_category=%s&_wpnonce=%s">'. __('Delete', $this->plugin_name) .'</a>', esc_attr( $_REQUEST['page'] ), 'delete', absint( $item['id'] ), $delete_nonce );
        }
