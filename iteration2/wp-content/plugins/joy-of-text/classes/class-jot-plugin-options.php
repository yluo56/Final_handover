...
final class Joy_Of_Text_Plugin_Options {
    ...
    public function process_group_invite_form() {
        ...
        if ($error==0) {
                ...
                $jot_grpinvrettxt  = isset($groupfields['jot_grpinvrettxt'])  ? sanitize_text_field ($groupfields['jot_grpinvrettxt'])  : "";
                ...
                if ( $invite_exists ) {
                        ...
                        $jot_grpgdprtxt = isset($groupfields['jot_grpgdprtxt']) ? $groupfields['jot_grpgdprtxt'] : "";
                        Joy_Of_Text_Plugin()->settings->save_groupmeta($groupfields['jot_grpid'], 'jot_grpgdprtxt', $jot_grpgdprtxt );
                        
                        // Save GDPR notice checkbox
                        $jot_grpgdprchk = isset($groupfields['jot_grpgdprchk']) ? true : false;
                        Joy_Of_Text_Plugin()->settings->save_groupmeta($groupfields['jot_grpid'], 'jot_grpgdprchk', $jot_grpgdprchk );
    ...
    public function process_subscriber_form() {     
        ...
        // Spam bot check
        $jot_subscribe_special = isset($output['jot-subscribe-special']) ? wp_strip_all_tags($output['jot-subscribe-special']) : "";
    ...
    public function process_add_member($param_memname = null, $param_memnum = null, $param_grpid = null) {
        ...
        $response = array('errormsg'=> $msg, 'errorcode' => $error, 'errorfield' => $errorfield,'url'=> "", 'sqlerr' => $wpdb->last_error, 'lastid'=> $lastmemid, 'verifiednumber' => $verified_number );
    ...
    public function process_delete_member() {
        ...
        if ( $error==0 ) {
                ...
                // Get member details
                $member = Joy_Of_Text_Plugin()->messenger->get_member($formdata['jot_grpmemid']);
                
                // Delete member
                $success=$wpdb->delete( $table_mem,  array( 'jot_grpid' =>  $formdata['jot_grpid'],'jot_grpmemid' => $formdata['jot_grpmemid'] ) );
                $success=$wpdb->delete( $table_xref, array( 'jot_grpid' =>  $formdata['jot_grpid'],'jot_grpmemid' => $formdata['jot_grpmemid'] ) );
                
                if ($success > 0) {
                    // Action hook for deleted user                    
                    $group = (array) Joy_Of_Text_Plugin()->settings->get_group_details($formdata['jot_grpid']);
     ...
     public function process_download_group() {
            ...
            if (!empty($jot_grpid)) {
               ...
               foreach ( $groupmembers as $member ) {
                   ...
                   $data .= $this->addQuotes((isset($member->jot_grpmemname)    ? $member->jot_grpmemname    : "") ) . "," .
                            $this->addQuotes((isset($member->jot_grpmemnum)     ? $member->jot_grpmemnum     : "") ) .
                            "\r\n";
               }
